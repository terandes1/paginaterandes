	...
	<div class="form form-spac rows">
		<div class="container">
			...
			<div class="spe-title col-md-12">
				...
				<p id="subtitulo">Lands of the Andes world leader in tours, more than 30,000 packages around the world. Book travel packages and enjoy your vacation with a unique experience</p>
			</div>
			<div class="col-md-6 col-sm-6 col-xs-12 form_1 wow fadeInLeft" data-wow-duration="1s">
				...
				<div class="succ_mess">Thank you for contacting us we will get back to you soon.</div>
					...
					<ul>
						<li>
							<input type="text" name="name"  id="name" value="{{ old('name') }}" placeholder="Write your name" required>
							<p class="errorValidacion">{{ $errors->first('name') }}</p>
						 </li>
						<li>
							<input type="tel" name="telefono"  id="telefono" value="{{ old('telefono') }}" placeholder="Telephone number" required> 
						...
						<li style="width: 100%;text-align: center;">
							<textarea class="form-control" name="message"  cols="120" rows="3"  placeholder="Enter your message">{{ old('message') }}</textarea>
							<p class="errorValidacion">{{ $errors->first('message') }}</p>
							
										{!! Recaptcha::render() !!}
 										<p class="errorValidacion">{{ $errors->first('g-recaptcha-response') }}</p>
							<input  style="text-align: center;" type="submit" value="Submit" id="send_button">
						</li>							
					</ul>
				{!! Form::close() !!}  <br>
			</div>

	...
	<div class="form form-spac rows">
		<div class="container">
			...
			<div class="spe-title col-md-12">
				<h2 id="titulo">CONTATTACI</h2>
				<div class="title-line">
					<div class="tl-1"></div>
					<div class="tl-2"></div>
					<div class="tl-3"></div>
				</div>
				<p id="subtitulo">Tierras de los andes leader mondiale nei tour, oltre 30.000 pacchetti in tutto il mondo. Prenota pacchetti di viaggio e goditi la tua vacanza con un'esperienza unica</p>
			</div>
			<div class="col-md-6 col-sm-6 col-xs-12 form_1 wow fadeInLeft" data-wow-duration="1s">
				...
				<div class="succ_mess">Thank you for contacting us we will get back to you soon.</div>
					...
					<ul>
						<li>
							<input type="text" name="name"  id="name" value="{{ old('name') }}" placeholder="Scrivi il tuo nome" required>
							<p class="errorValidacion">{{ $errors->first('name') }}</p>
						 </li>
						<li>
							<input type="tel" name="telefono"  id="telefono" value="{{ old('telefono') }}" placeholder="Numero di telefono" required> 
						...
						<li>
							...
							<input type="hidden" name="idioma"  id="idioma" value="{{$abbr}}" value="{{ old('idioma') }}"> 
						...
						<li style="width: 100%;text-align: center;">
							<textarea class="form-control" name="message"  cols="120" rows="3"  placeholder="Inserisci il tuo messaggio">{{ old('message') }}</textarea>
							<p class="errorValidacion">{{ $errors->first('message') }}</p>
							
										{!! Recaptcha::render() !!}
 										<p class="errorValidacion">{{ $errors->first('g-recaptcha-response') }}</p>
							<input  style="text-align: center;" type="submit" value="Inviare" id="send_button">
						</li>							
					</ul>
				{!! Form::close() !!}  <br>
			</div>
